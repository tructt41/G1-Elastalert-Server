...
##
    ...
    def start(self):       
        ...
        while self.running:
            ...
            #################
            ouput_filename = rule['name']
            log_file = f"./result_elastalert/{ouput_filename}.json"
...
def handle_signal(signal, frame):
    ...
    os._exit(0)
##############################################
def print_rule_name(rule):
    return rule['name']
##############################################
def get_logger(ouput_filename):
    log_file = f"./result_elastalert/{ouput_filename}.json"
    logger = logging.getLogger('ElastAlerter')
...
def get_logger_all():
    ...
    loggerall = logging.getLogger('ElastAlerter')
    ...
    loggerall.addHandler(console_handler)

    return loggerall
elastalert_logger = get_logger_all()
...
def main(args=None):
    ...
    if not client.args.silence:
        for rule in client.rules: 
            ouput_filename = print_rule_name(rule)
            elastalert_logger = get_logger(ouput_filename) 
        client.start()
